<template>
  <div class="algtrainer">
    ...
    <scramble :scramble='item.alg' :move='move' @execMoves='executeMoves' ref='scramble' />
    <div id='twisty'></div>
    <div>{{elapsedTime}}</div>
    <div>{{item}}</div>
  </div>
  ...
  methods: {
    async connect () {
      ...
      window.puzzle.addMoveListener((e) => {
        ...
        this.move = { move: e.latestMove, key: Math.random() } // we need to set up a random key so that the watcher in Scramble is updated (eg. U2 = U + U. we need a different key to ensure the moves are separated)
  ...
  mounted () {
    ...
    document.addEventListener('keydown', (e) => {
      if ('urfdlbURFDLB'.split('').includes(e.key)) {
        ...
        this.move = { move: m, key: Math.random() }
